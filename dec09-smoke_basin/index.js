...
class Location {
  constructor (input) {
    ...
    this.context = input.context.flatMap(el => el)
  }
...
function nineboxContext (floor, rowIdx, colIdx) {
  ...
  return floor
    .slice(...contextShape.breadth)
    .map(row => row.slice(...contextShape.width))
}
...
export default function * pickPart (input, config) {
  ...
  const summary = []
  if (config.part === 2) {
    config.getContext = nineboxContext
